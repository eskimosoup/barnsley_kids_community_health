module Optimadmin
  class ContactDetailsController < Optimadmin::ApplicationController
    ...
    def index
      @contact_details = Optimadmin::BaseCollectionPresenter.new(collection: ContactDetail.where('name ILIKE ?', "#{params[:search]}%").page(params[:page]).per(params[:per_page] || 15), view_template: view_context, presenter: Optimadmin::ContactDetailPresenter)
    end
